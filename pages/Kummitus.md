...
**来源:** https://noita.wiki.gg/zh/wiki/Kummitus
---
...
[ ![Monster playerghost stand full.gif](https://noita.wiki.gg/zh/images/Monster_playerghost_stand_full.gif?ff3559) ](/zh/wiki/File:Monster_playerghost_stand_full.gif "Monster playerghost stand full.gif") _"我所逝去的青春"_

![Monster playerghost.png](https://noita.wiki.gg/zh/images/thumb/Monster_playerghost.png/40px-Monster_playerghost.png?b302ba)

## 生成地点

### 普通

[矿场](矿场.md), [坍塌矿场](坍塌矿场.md), [煤矿矿坑](煤矿矿坑.md), [真菌洞穴](真菌洞穴.md), [积雪深渊](积雪深渊.md), [地下丛林](地下丛林.md), [避难所](避难所.md), [艺之神殿](艺之神殿.md), [冻结避难所](冻结避难所.md), [蜘蛛巢穴](蜘蛛巢穴.md), [巫师巢穴](巫师巢穴.md) (请看下列的生成区域)
...
## 掉落物
  ...
  * [500 ](黄金.md) [![Gold nugget.png](https://noita.wiki.gg/images/thumb/Gold_nugget.png/16px-Gold_nugget.png?1b79f5)](/zh/wiki/%E9%BB%84%E9%87%91 "黄金")



**鬼影** （游戏内名为**Kummitus** 或古迷图斯，中文社区称其为玩家幽灵或老古）是一种半稀有的会飞行的生物，与[玩家](米纳.md)的长相十分相似，通常会出现在[矿场](矿场.md)、[煤矿矿坑](煤矿矿坑.md)或[积雪深渊](积雪深渊.md)里面。 

## 行为

鬼影总是会携带着一根魔杖生成，在死亡后会掉落。如果魔杖内没有可用的法术了，那么它可能会故意丢掉魔杖。如果鬼影手上没有魔杖的话，它将试图踢玩家，造成一定程度的伤害。它对**所有** 生物都怀有敌意，可以利用这一点，用一些创造性的风筝战帮助消灭特别麻烦的敌人。然而需要注意的是，如果玩家在它的视线范围内，鬼影会有把攻击目标换成玩家的倾向。 
...
## 战斗技巧
  ...
  * 作为一个亡灵类的敌人，它免疫材料给予的沾染效果，包括[变形魔药](变形魔药.md)。但是这并不是代表有变形免疫，基于法术的变形效果仍然可以使它变形。
  * 它并不会被强气流击退.
...
鬼影有一套专属的生成机制。其中的一个必须条件是玩家**必须** 在游戏中已经死亡了35次。下列是剩余条件： 

  1. 这**35次死亡** 内并没有开启模组，
  2. 你并不是因为点金伤害导致的死亡，[[1]](#cite_note-delete-1)
  3. 在变形后如果发生了死亡将不算数。
  4. 死亡时玩家必须携带至少有一根编辑过的魔杖[[2]](#cite_note-2)。



### 尸骨文件

每次在这种情况下的死亡都会在存档内的长久性保存数据中产生一个尸骨文件[[3]](#cite_note-3)。尸骨文件本质上只是魔杖实体的数据，存储在单独的XML文件中，当文件夹内有 50 个文件时，添加新文件的时候将随机删除多余的文件。当玩家手持多根魔杖死亡时，会从有效的魔杖中随机挑选一根出来存储到尸骨文件中。 

  * `**这是确切路径** ：/Nolla_Games_Noita/save00/persistent/bones_new`



你可以使用这个 _不安全[[4]](#cite_note-4)_的模组在游戏里编辑和加载其中的法杖： 

  * <https://github.com/KagiamamaHIna/kummitus-wand-viewer/releases/latest>



Alter_Ukko还制作了一个用于检查尸骨文件的工具，因此你可以用于浏览这些文件并删除无聊的尸骨文件。 

  * 图片：<https://gitlab.com/alter_ukko/noita-util/-/raw/main/screenshots/bone_wand_screen.png>
  * 全文： <https://gitlab.com/alter_ukko/noita-util/-/releases>
  * 说明： <https://gitlab.com/alter_ukko/noita-util>



### 生成触发机制

与其他敌人的生成触发器不同，鬼影有一个单独的生成函数，该函数与一个非常特殊的生成触发器相关联，其他所有生物都不会使用此生成触发器。这个生成触发器有五种状态[[5]](#cite_note-5)，在这些状态下它会做不同的事情： 
...
当状态为0时，每次加载生成点时都有机会前进到下一个状态。 发生这种情况的可能性取决于户外雾的数量（特别是[WorldStateComponent](https://noita.wiki.gg/wiki/Documentation:%20WorldStateComponent)上的fog属性）。如果雾度低于或等于10%，则有3.33%的机会进入下一个状态，如果雾度高于10%，则机会增加到8.33%。 从状态0前进到状态1的检查可能会失败好几次，将导致整局游戏中没有鬼影生成。该函数有一个与种子和坐标都无关的随机变量，这导致即使在同一个种子中，鬼影的生成区域也会发生很大变化。如果检查成功，游戏将从状态0推进到状态2，几乎是在瞬间内完成的。状态2和状态3持续的时间与加载下一个可能的鬼影生成地点需要的时间一样长。 

### 生成区域

下列表格中列出了鬼影当前可以或不可以在哪些生物群系中生成，以及当前在对应生物群系中生成位置的数量[[6]](#cite_note-6) 。鬼影的生成概率是未知的，但一般来说，给定生物群系中的可能的生成位置越多，生物群系越大，在该生物群系中看到鬼影的可能性就越大。  
...
[砂之洞穴](砂之洞穴.md) | 不会生成   
[云景](云景.md) | 不会生成   
[伟大之作(天空)](https://noita.wiki.gg/zh/wiki/%E4%BC%9F%E5%A4%A7%E4%B9%8B%E4%BD%9C%5C(%E5%A4%A9%E7%A9%BA%5C) "伟大之作\(天空\)") | 不会生成[[8]](#cite_note-the_end-8)  
[伟大之作(地狱)](https://noita.wiki.gg/zh/wiki/%E4%BC%9F%E5%A4%A7%E4%B9%8B%E4%BD%9C%5C(%E5%9C%B0%E7%8B%B1%5C) "伟大之作\(地狱\)") | 不会生成[[8]](#cite_note-the_end-8)  
  
## 你知道吗？

  * 因为鬼影是作为玩家的幽灵来出现的，所以它总是携带着玩家当时死亡时所使用的魔杖之一。在游戏内部文件中，这个实体被命名为playerghost（玩家的鬼魂）。
  * 虽然大多数敌人都是以满生命值上限状态的情况下生成，但鬼影在生成时只有：
...
因此，它掉落的黄金比拥有250点血上限的敌人要多。这样做的一个副作用是：鬼影可以利用它所携带的魔杖中的治疗法术来进行自我治疗。
  * 使用steam云存档时，鬼影有时没有法杖。这是因为游戏试图读取`steam_autocloud.vdf`中的尸骨文件。



## 备注

  1. [↑](#cite_ref-delete_1-0) 这将在保存前将魔杖删除
  2. [↑](#cite_ref-2) 也就是说，如果玩家携带的是未经修改过的魔杖将不算数
  3. [↑](#cite_ref-3) 尸骨文件被储存在存档save00文件夹中的persistent/bones_new文件夹内。
  4. [↑](#cite_ref-4) 由于安全模组无法读取存档里的文件，所以只能是不安全模组，并且不安全模组无法上传创意工坊
  5. [↑](#cite_ref-5) 状态值在save00/world_state.xml中可见，在一个名为 "apparitions_per_level"的数组中。
  6. [↑](#cite_ref-6) 这些信息是用这个[PowerShell脚本](https://gist.github.com/vexx32/cfa23f44edfe4f29ffdcc4c8a77ccbfe)从wang数据中找出来的，在未来的更新中可能会有所变化。
  7. [↑](#cite_ref-7) 地下丛林在技术上被分割成了两个类似的生物群系，其中一个会生成更宽阔的区域。两个子生物群系中各有6个生成位置，但请记住，地下丛林的总面积大约一半是正常的丛林，另一半是"宽敞"的丛林，所以这里的额外的生成位置的来源主要是因为生物群系的整体实现方式所增加的。
  8. ↑ [8.0](#cite_ref-the_end_8-0) [8.1](#cite_ref-the_end_8-1) 被称为**伟大之作** 的重复生物群系在它们的wang数据中有6个鬼影生成触发器，但在它们的生物群系脚本中明确地用一个无操作函数覆盖了spawn_apparition()函数。这使得鬼影无法在这些区域中生成。
