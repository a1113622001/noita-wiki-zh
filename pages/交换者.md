...
**分类:** [Category:具有正数的施放延迟法术](Category%3A具有正数的施放延迟法术.md) · [Category:影响存在时间的修正](Category%3A影响存在时间的修正.md) · [Category:影响散射角度的修正](Category%3A影响散射角度的修正.md) · [Category:拥有暴击概率加成的法术](Category%3A拥有暴击概率加成的法术.md) · [[Category:没有标签的法术]] · [Category:Spells](Category%3ASpells.md) · [Category:投射物法术](Category%3A投射物法术.md)
**来源:** https://noita.wiki.gg/zh/wiki/%E4%BA%A4%E6%8D%A2%E8%80%85
---

![Spell quantum split.png](https://noita.wiki.gg/zh/images/thumb/Spell_quantum_split.png/18px-Spell_quantum_split.png?8dfaa6) | _本条目的主题是交换者法术。关于会发射交换魔弹的交换法师，见[交换法师](交换法师.md)。_  
---|---  
...
传送法术   
---  
[](传送魔弹.md) [](传送魔弹.md) [](返回.md) [](##) [](抓取魔弹.md)  
涉及位移的法术，能够影响施法者，目标或两者。   
  
**移形换位** （又名**交换者** 、**交换魔弹** ）是一个投射物法术。它能够交换被它击中的生物和施法者的位置。和[传送魔弹](传送魔弹.md)不同，它传送时不会检查目的地是否有足够的空间，这使得使用它有一定风险，同时因此它也是[跃迁](跃迁.md)的常用法术之一。 

## 使用技巧

  * 如果两个交换者几乎同时击中了目标，那么它们的效果会相互抵消，导致最终看起来没有发生传送。
  * 使用[远距离施放](远距离施放.md)，搭载的载荷中同时施放交换者、伤害修正（比如[轻盈一击](轻盈一击.md)）和[召唤鱼](召唤鱼.md)，可以让你往墙壁中进行传送。 
    * 可以添加[零时](零时.md)用于移除鱼。
    * 召唤鱼可以用[召唤诱饵鹿](召唤诱饵鹿.md)替换，但是这样尸体会更多。
    * [零时](零时.md)修正下的[投射物转化领域](投射物转化领域.md)，也能达到近似的效果。
    * 一个简单的示例配置如下所示：
    ...
    |spread=0
    |spellsCast=1
    |spells=LONG_DISTANCE_CAST,LIGHT_SHOT,NOLLA,BURST_2,SWAPPER_PROJECTILE,FISH
    |alwaysCasts=
    }}
...
[法杖模拟器](https://tinker-with-wands-online.vercel.app/?a=1&c=2500&r=20&d=10&v=1&x=0&m=5000&l=6&q=0&s=LONG_DISTANCE_CAST%2CLIGHT_SHOT%2CNOLLA%2CBURST_2%2CSWAPPER_PROJECTILE%2CFISH)
...
[](召唤鱼.md)
  ...
  * 携带一个[屏息静气](屏息静气.md)天赋可以防止你在墙壁中窒息。 
    * 并不是所有敌人都需要呼吸，但是你可以用交换的方式让需要呼吸的敌人在墙壁中窒息而死。
  * 交换者可以无视距离和区块加载进行传送。 
    * 利用这一点可以进行远距离的传送，甚至瞬间抵达另一个[平行世界](平行世界.md)。详见[专家攻略：跃迁配置/超传配置](专家攻略：跃迁配置%2F超传配置.md)。
  * 交换者可以击中的实体需要带有 `hittable` 标签和以下标签之一：`enemy`、`prey`、`effectable_prop`、`hiteffect_enabled`。 
    * 交换的目标不需要受到伤害。
    * 为了能击中我们自行召唤的生物（比如鹿和鱼），我们需要交换者携带非 0 的伤害，这可以通过加伤/减伤的修正实现。[[1]](#cite_note-1)
  * 和[传送魔弹](传送魔弹.md)一样，交换者使用一个[缩减存在时间](缩减存在时间.md)就能[永久化](专家攻略：无限持续时间法术.md)。
  * 触发搭载带追踪的交换者可以用于将敌人搬运到圣山中。先施放触发弹再在交换者生效之前进入圣山即可。



注：

  1. [↑](#cite_ref-1) 部分情况下，通过加减伤害让伤害重新回到 0 有时还是能够击中，这有可能是 IEEE754 浮点数精度误差导致的。



## 你知道吗？

  * 在交换者击中目标的同一瞬间失效的各类传送魔弹会在交换者之后生效。被击中的目标会被传送到你之前的位置，而你被传送到其他传送魔弹失效的位置。
  * 这个法术与 Noita 的开发者之一在 2013 年参与制作的一款独立游戏《交换者（The Swapper）》同名。参见 Steam 上的 [The Swapper 商店页面](https://store.steampowered.com/app/231160/The_Swapper/)。
  * [换位法师](Vaihdosmestari.md)会使用这种投射物。



## 画廊

[](https://noita.wiki.gg/zh/wiki/File%3ADemo%20Swapper%20Range%20noita.mp4)

交换者的默认射程

[](https://noita.wiki.gg/zh/wiki/File%3ADemo%20Swapper%20Range%20With%20Nolla.mp4)

零时交换者的射程

[](https://noita.wiki.gg/zh/wiki/File%3ADemo%20Swapper%20Range%20With%20ReduceLife.mp4)

红表交换者的射程

## 历史

  * **[2024年04月08日(Epilogue2)](版本日志.md)**: 将 2、4、5、6 级的生成权重从 0.05 分别调整为 0.1、0.4、0.4、0.1。
