...
**分类:** [Category:需要补充图片的页面](Category%3A需要补充图片的页面.md) · [Category:适用于最新版的页面](Category%3A适用于最新版的页面.md) · [Category:具有正数的施放延迟法术](Category%3A具有正数的施放延迟法术.md) · [Category:影响存在时间的修正](Category%3A影响存在时间的修正.md) · [Category:影响散射角度的修正](Category%3A影响散射角度的修正.md) · [[Category:影响后坐力的修正]] · [Category:投射物伤害法术](Category%3A投射物伤害法术.md) · [[Category:没有标签的法术]] · [Category:Spells](Category%3ASpells.md) · [Category:投射物法术](Category%3A投射物法术.md)
**来源:** https://noita.wiki.gg/zh/wiki/%E6%8A%93%E9%92%A9%E9%AD%94%E5%BC%B9
---
...
**抓钩魔弹** 是一种投射物法术，能发射一种绿色的爪形投射物，失效时为玩家设置一个 800 像素/秒的瞬时速度，方向朝向抓钩魔弹的失效点。 

## 使用技巧

  * 虽然抓钩魔弹能造成[![投射物伤害](https://noita.wiki.gg/zh/images/Damage_Projectile_dmg-large.png?306896)](/zh/wiki/%E4%BC%A4%E5%AE%B3%E7%B1%BB%E5%9E%8B#投射物伤害 "投射物伤害")7.5的伤害，但是一般不会用它自己攻击敌人，因为那样会把玩家拉近敌人，较为危险。
  * 在没有[传送魔弹](传送魔弹.md)时，抓钩魔弹可以作为其下位使用，但它相较于传送魔弹，不能传送通过狭小缝隙和[神圣之山](神圣之山.md)的坍塌判定框，也不能不受伤害地越过敌人的子弹。
  * 由于抓钩魔弹失效时是**设置** 玩家速度，同一帧内有多个抓钩失效时，决定速度方向的是最后一个失效的抓钩。
  * 搭配[零时](零时.md)高频发射抓钩可以实现平滑稳定的飞行。
  * 将它加入基于零时[黑洞](黑洞.md)开路的[传送魔杖](https://noita.wiki.gg/zh/wiki/%E4%B8%93%E5%AE%B6%E6%94%BB%E7%95%A5%EF%BC%9A%E4%BC%A0%E9%80%81%E9%AD%94%E6%9D%96%3Faction%3Dedit%26redlink%3D1)中能加快传送魔杖的[挖掘](挖掘.md)速度，但也会导致传送魔杖略微发飘。


  * 将抓钩魔弹部署在 NaN 坐标会将玩家的速度设置为 NaN [[1]](#cite_note-1)，实现对玩家的定身效果，使玩家只能通过传送的方式进行移动。释放一个普通的抓钩魔弹能消除这种状态。参见[延时上冲](延时上冲.md)。 
...
[](向上飞行.md)
    ...
    * [远距离施放](远距离施放.md)可以替换为[定时火花弹](火花弹.md)，直接去掉也可以，但会导致在出生点附近无法使用。
    * [一分为四](一分为多.md)可以替换为一分为三。
    * 延时上冲可以替换为[延时下坠](延时下坠.md)。



## 你知道吗？

  * 抓钩魔弹为玩家设置速度的效果除了在失效时执行[[2]](#cite_note-2)，还被设置为每 999999 帧执行一次[[3]](#cite_note-3)。释放一个[永久化](专家攻略：无限持续时间法术.md)抓钩并在不重启游戏的情况下等待 1000000 帧（约 11.57 天），就能看到玩家被拉向抓钩魔弹的所在方向了。
...
## 注释
  ...
  1. [↑](#cite_ref-1) 这是因为抓钩魔弹先使用`atan2`函数，根据抓钩魔弹所在坐标与玩家坐标获取抓钩魔弹相对于玩家的角度，再通过这个角度把 800 像素/秒的总速度投影得到玩家的x、y分速度。 NaN 坐标的抓钩魔弹会导致`atan2`函数返回 NaN 角度，进而导致玩家的x、y分速度都被设定为 NaN 。
  2. [↑](#cite_ref-2) 代码上，这对应`[LuaComponent](Documentation%3A LuaComponent.md)`的`execute_on_removed=1`。
  3. [↑](#cite_ref-3) 代码上，这对应`[LuaComponent](Documentation%3A LuaComponent.md)`的`execute_every_n_frame=999999`。
